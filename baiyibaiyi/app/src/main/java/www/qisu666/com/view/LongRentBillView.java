    ...
    public void setData(BillLongRentResp billLongRentResp) {
        ...
        if (!StringUtils.isEmpty(comboExtraTime)) {
            ...
            if (!StringUtils.isEmpty(comboExtraMileageMoney)) {
                double mileageCost = Integer.parseInt(comboExtraMileageMoney) / 100.00;
                //折扣率
                int mileageDiscount = billLongRentResp.getMileageDiscount();
                if (mileageDiscount != 100) {
                    //原价
                    String originMileagePrice = TVUtils.toString(billLongRentResp.getMileageDiscountBeforeMoney()/100.0);
                    viewHolder.tvComboExtraMileageMoney.setText(originMileagePrice + "*" + TVUtils.toString1(mileageDiscount / 10.0 + "") + "折=" + TVUtils.toString(mileageCost) + "元");
                }else {
                    viewHolder.tvComboExtraMileageMoney.setText(TVUtils.toString(mileageCost) + "元");
                }
            }
            ...
            if (!StringUtils.isEmpty(comboExtraTimeMoney)) {
                double timeCost = Integer.parseInt(comboExtraTimeMoney) / 100.00;
                //折扣率
                int timeDiscount = billLongRentResp.getTimeDiscount();
                if (timeDiscount != 100) {
                    //原价
                    String originTimePrice = TVUtils.toString(billLongRentResp.getTimeDiscountBeforeMoney()/100.0);
                    viewHolder.tvComboExtraTimeMoney.setText(originTimePrice + "*" + TVUtils.toString1(timeDiscount / 10.0 + "") + "折=" + TVUtils.toString(timeCost) + "元");
                }else {
                    viewHolder.tvComboExtraTimeMoney.setText(TVUtils.toString(timeCost) + "元");
                }
            }
        } else {
            //隐藏套餐外信息
