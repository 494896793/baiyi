...
//            tvVerifyIdDesc.text = Html.fromHtml(HighlightUtil.convertHightlightText("包含身份证、驾驶证，已认证", "已认证", "#59d66f"))
        ...
        if (deposit == 0 || deposit ?: 0 < shouldDeposit ?: 0) {
            ...
            if (deposit == 0) {//未交纳保证金
                ...
                tvVerifyPledgeDesc.text = "需交${shouldDeposit?.div(100)}"
            } else {//未交纳全部保证金
                tvVerifyPledgeDesc.text = Html.fromHtml(HighlightUtil.convertHightlightText("已交${deposit?.div(100)},还需${(shouldDeposit?.minus(deposit ?: 0))?.div(100)}", "还需${(shouldDeposit?.minus(deposit ?: 0)?.div(100))}", "#main_info_color"))
            }
            money_statu_tx.text="未交纳"
        } else {//已交纳保证金
            tvVerifyPledgeDesc.setTextColor(ContextCompat.getColor(mContext, R.color.main_info_color))
            tvVerifyPledgeDesc.text = "已交${deposit?.div(100)}"
            money_statu_tx.text="已交纳"
        }
    }
