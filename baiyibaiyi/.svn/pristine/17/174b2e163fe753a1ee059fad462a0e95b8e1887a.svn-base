...
//    }
        ...
        //右上角的已选数量
        tvRightMenu = (TextView) findViewById(R.id.tvTitleRight);
        LinearLayout llytTitleRight = (LinearLayout) findViewById(R.id.llytTitleRight);
        llytTitleRight.setBackgroundResource(R.drawable.__picker_bg_white_radius);
        ...
        pickerFragment.getPhotoGridAdapter().setOnItemCheckListener(new OnItemCheckListener() {
            ...
            public boolean OnItemCheck(int position, Photo photo, final boolean isCheck, int selectedItemCount) {
                ...
                boolean isEnable = total > 0;
                tvRightMenu.setEnabled(isEnable);
                if (isEnable) {
                    tvRightMenu.setBackgroundResource(R.drawable.__picker_bg_blue_radius_enable);
                } else {
                    tvRightMenu.setBackgroundResource(R.drawable.__picker_bg_blue_radius_unenable);
                }
     ...
     * 判断图片的选择状态
     */
    private void judgePhoneSelectStatus() {
        List<String> tmp = new ArrayList<String>();
        for (String str : originalPhotos) {
            if (!str.startsWith("http")) {
                tmp.add(str);
            }
        }

        if (tmp.size() > 0) {//已有照片被选中
            tvRightMenu.setEnabled(true);
            tvRightMenu.setText("完成(" + tmp.size() + ")");
            tvRightMenu.setBackgroundResource(R.drawable.__picker_bg_blue_radius_enable);
        } else {//没有照片被选中
            tvRightMenu.setEnabled(false);
            tvRightMenu.setText("完成");
            tvRightMenu.setBackgroundResource(R.drawable.__picker_bg_blue_radius_unenable);
        }
    }
...
//    }
    ...
    public void updateSelectPhone(ArrayList<String> paths) {
        //更新相册勾选状态
        originalPhotos = paths;
        judgePhoneSelectStatus();
    }
}
